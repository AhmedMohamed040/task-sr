...
export default function InductionSiteView({ tab }: Props) {
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'xl'}>
      ...
      <Box mt={3}>
        {currentTab === 'sports-services' && <>sports services</>}
        {currentTab === 'standards-management' && <>standards management</>}
        {currentTab === 'centers-settings' && <>centers settings</>}
        {currentTab === 'job-roles' && <>job roles</>}
        {currentTab === 'terms-and-conditions' && <>terms and conditions</>}
        {currentTab === 'about-plantform' && <>about plantform</>}
      </Box>
    </Container>
  );
}
