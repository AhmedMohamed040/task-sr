...
export default function ChatRoom({ participants, conversation }: Props) {
  ...
  const { messagesEndRef } = useMessagesScroll(participants);
  ...
  const attachments = uniq(flatten(conversation?.map((messages:IChatMessage) => messages?.attachment)));

  const renderContent = (
    <>
      <ChatRoomSingle participant={participants[0]} />

      <ChatRoomAttachments attachments={attachments as any} />
    </>
  );
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      {lgUp ? (
        ...
        >
          <Scrollbar ref={messagesEndRef} sx={{  py: 5, height: 1 }}>
