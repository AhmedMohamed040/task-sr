...
import SportsServies from './tabs/sports-servies';
import StandardsManagement from './tabs/Standards-Management';
import CentersSettings from './tabs/centers-settings';
import AboutPlatform from './tabs/about';
import JobRoles from './tabs/job-roles';
...
export default function SportsView({ tab }: Props) {
  const { t } = useTranslate();
  const settings = useSettingsContext();

  const currentTab = useMemo(
    () =>
      typeof tab === 'string' && tabs.find((item) => item.value === tab) ? tab : 'sports-services',
    [tab]
  );
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'xl'}>
      <Tabs value={currentTab} onChange={handleChangeTab} aria-label="icon position tabs example">
        {tabs.map((item) => (
          <Tab
            key={item.value}
            value={item.value}
      ...
      <Box mt={3}>
        {currentTab === 'sports-services' && <SportsServies />}
        {currentTab === 'standards-management' && <StandardsManagement />}
        {currentTab === 'centers-settings' && <CentersSettings />}
        {currentTab === 'job-roles' && <JobRoles />}
        {currentTab === 'terms-and-conditions' && <AboutPlatform />}
        {currentTab === 'about-plantform' && <AboutPlatform />}
      </Box>
    </Container>
  );
}
