...
export default function ChatRoomSingle({ participant }: Props) {
  ...
  const renderInfo = (
    <Stack alignItems="center" sx={{ py: 5 }}>
      <Avatar alt={user?.name || t('Unknown')} src={user?.avatar || ''} sx={{ width: 96, height: 96, mb: 2 }} />
      <Typography variant="subtitle1">{user?.name || t('Unknown')}</Typography>
  ...
  const renderContent = (
    ...
    {/*   <Stack direction="row">
      ...
      <Stack direction="row">
        ...
        <Typography variant="body2">{user?.phone || '+966'}</Typography>
      </Stack>
